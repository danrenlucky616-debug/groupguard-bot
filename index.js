...
async function sendWarning(sock, groupId, userId, reason) {
  ...
  if (user.warnings >= WARN_LIMIT) {
    ...
    try {
      ...
      console.log('Could not remove user:', error.message)
    }
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        console.log('🔐 WhatsApp logged out. Scan again.')
      }
    }
  })
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const participant =
      metadata.participants.find(p => p.id === sender)
    ...
    const hasLink =
      /(https?:\/\/|www\.|(?:^|\s)\S+\.(com|net|org|ng|io)\b)/i.test(text)
